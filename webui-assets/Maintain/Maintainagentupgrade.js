...
export default class Maintainagentupgrade extends React.Component {
                ...
                response.json().then((gateways) => {
                    ...
                    if(gateways.length > 0){
                        for(let gateway of gateways){
                            ...
                            prepareData.environment = gateway.environment;
    ...
    deleteData(tbodyIndex) {

        let cnf = window.confirm('Are you sure you want to delete');
        let rowdata = this.state.newTableData;
        if (cnf) {
            fetch(this.state.apiEndPoints.baseUrl)
                .then((response) => {
                    if (response.status === 200) {
                        response.json().then((zones) => {
                            this.props.showGlobalMessage(true, true, 'Record deleted successfuly', 'custom-success');
                            let that = this;
                            setTimeout(function () {
                                that.props.hideGlobalMessage();
                            }, 2000);

                            const newRows = rowdata.slice(0, tbodyIndex).concat(rowdata.slice(tbodyIndex + 1));
                            this.setState({
    ...
    render() {
        return (
                                    ...
                                    <tr key={'maintainagentupgradeTableTbodyTr_'+tbodyIndex}>
                                        ...
                                        <td>
                                            <span className="action-img">
                                                ...
                                                <img alt="disable-icon" onClick={this.disbableData.bind(this, tbodyIndex )} title="Disable" src="assets/static/images/icondisable_tablemaintain.svg" />
                                                <img alt="delete-icon" onClick={this.deleteData.bind(this, tbodyIndex)} title="Delete" src="assets/static/images/icondelete_tablemaintainmonitor.svg" />
                                            </span>
                                        </td>
                                    </tr>
                                )
            ...
            :
            <Maintainagentedit showGlobalMessage={this.props.showGlobalMessage.bind(this)} hideGlobalMessage={this.props.hideGlobalMessage.bind(this)} changeView={this.changeView.bind(this)} editItemData={this.state.editItemData}></Maintainagentedit>
        }
        </div>
        )
    }
